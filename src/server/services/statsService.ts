import { fetchDashboardData, tableCount, type DashboardRaw } from "@/server/repositories/statsRepo";
...
/**
 * Pure aggregation over the raw dashboard datasets. Separated from I/O so it
 * can be unit-tested with fixtures.
 */
export function aggregateDashboard(
    raw: DashboardRaw,
    counts: { subscribers: number; leads: number; contacts: number; conversations: number; opportunities: number; accounts: number }
): DashboardStats {
    ...
    const views = raw.views.reduce((s, r) => s + (Number(r.views) || 0), 0);
    const opps = raw.opps;
    ...
    // Rep leaderboard.
    const repNames = raw.reps.map((r) => r.name);
    ...
    return {
        ...
        newLeads14d, stageBreakdown,
        recentSubs: raw.recentSubs,
        recentContacts: raw.recentContacts,
        hotOpportunities: raw.hot,
        repLeaderboard,
        updatedAt: new Date().toISOString(),
    };
}

/** Live snapshot for the admin dashboard. Tolerates not-yet-created tables and
 * an unconfigured database (returns an empty snapshot). */
