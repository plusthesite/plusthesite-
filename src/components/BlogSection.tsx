...
export default function BlogSection() {
    ...
    return (
        <section id="blog" className="bg-white py-24 text-slate-950 lg:py-32 dark:bg-slate-950 dark:text-white">
                ...
                <div className="grid gap-10 lg:grid-cols-[0.92fr_1.08fr] lg:items-end">
                    ...
                    <div className="fade-up fade-up-delay-2 rounded-[1.75rem] border border-slate-200 bg-slate-50 p-6 dark:border-white/10 dark:bg-white/[0.04]">
                        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
                            {locale === "id" ? "Editorial pulse" : "Editorial pulse"}
                        </p>
                        ...
                        >
                            <div className="relative h-52 w-full overflow-hidden">
                                <Image
                                    ...
                                    unoptimized
                                />
                                <div className={`absolute inset-0 ${index === 0 ? "bg-gradient-to-t from-slate-950/80 to-transparent" : "bg-gradient-to-t from-slate-950/40 to-transparent"}`} />
                                <span
                                    className={`absolute left-4 top-4 rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-wider backdrop-blur-sm ${
                                        index === 0
                                            ? "bg-white/12 text-white"
                                            : "bg-white/90 text-blue-700 dark:bg-slate-950/90 dark:text-blue-300"
                                    }`}
                                >
                                    {article.category}
                                </span>
                            ...
                            <div className="flex flex-1 flex-col p-6">
                                <div className="flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.14em]">
                                    <span className={index === 0 ? "text-white/55" : "text-slate-500 dark:text-slate-400"}>
                                    ...
                                    <span className={index === 0 ? "text-white/35" : "text-slate-300 dark:text-white/15"}>•</span>
                                    <span className={index === 0 ? "text-white/55" : "text-slate-500 dark:text-slate-400"}>
                                        {article.readTime} {t.blogSection.readSuffix}
                                    </span>
                                </div>

                                <h3 className={`mt-4 text-xl font-semibold leading-snug tracking-[-0.02em] line-clamp-2 ${index === 0 ? "text-white" : "text-slate-950 dark:text-white"}`}>
                                    {article.title}
                                </h3>
                                <p className={`mt-3 flex-1 text-sm leading-6 line-clamp-3 ${index === 0 ? "text-white/72" : "text-slate-600 dark:text-slate-300"}`}>
                                    {article.description}
                                </p>

                                <div className={`mt-8 flex items-center gap-2 text-sm font-semibold ${index === 0 ? "text-white" : "text-slate-950 dark:text-white"}`}>
