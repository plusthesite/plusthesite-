    ...
    items: [
      {
        href: "/admin/accounts",
        label: "Accounts",
        icon: "M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0H5m14 0h2M5 21H3m4-14h2m-2 4h2m-2 4h2m4-8h2m-2 4h2m-2 4h2",
      },
      {
        href: "/admin/leads",
        label: "Leads",
...
export function AdminNav({ role = "admin" }: { role?: Role }) {
  ...
  const visibleSections = sections
    .map((sec) => ({
      ...sec,
      items: sec.items.filter((it) => canAccess(role, it.href)),
    }))
      ...
      <form action="/admin/search" method="get" className="mb-3">
        <div className="relative">
          ...
          <input
            name="q"
            placeholder="Search..."
            className="w-full rounded-2xl border border-white/10 bg-white/8 py-2.5 pl-9 pr-3 text-sm text-slate-100 placeholder-slate-500 backdrop-blur-sm focus:border-sky-400 focus:outline-none focus:ring-2 focus:ring-sky-400/20"
          />
        </div>
      </form>
