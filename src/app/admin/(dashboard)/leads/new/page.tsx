...
export default function NewLeadPage() {
    return (
        <div className="max-w-2xl">
            ...
            <h1 className="mt-3 text-2xl font-bold text-slate-900">New Lead</h1>

            <form action={createLead} className="mt-6 space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="grid gap-4 sm:grid-cols-2">
                    <div><label className={label}>Name</label><input name="name" className={field} placeholder="Contact name" /></div>
                    ...
                    <div><label className={label}>Est. value (IDR)</label><input name="value" type="number" min="0" className={field} placeholder="8000000" /></div>
                    <div><label className={label}>Owner</label><input name="owner" className={field} placeholder="Sales rep" /></div>
                </div>
                <div><label className={label}>Notes</label><textarea name="message" rows={3} className={field} placeholder="Context…" /></div>
                <input type="hidden" name="locale" value="id" />
                <div className="flex justify-end gap-2">
                    <Link href="/admin/leads" className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50">Cancel</Link>
                    <button className="rounded-lg bg-blue-600 px-5 py-2 text-sm font-semibold text-white hover:bg-blue-700">Create Lead</button>
                </div>
            </form>
        </div>
    );
