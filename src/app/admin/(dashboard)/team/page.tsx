...
export default async function TeamPage() {
    ...
    for (const row of (oppsRaw ?? []) as {
        owner: string | null;
        value: number;
        stage: string;
    }[]) {
        if (row.owner && row.stage !== "lost") {
            const current = oppsByOwner.get(row.owner) ?? { count: 0, value: 0 };
            current.count++;
            current.value += Number(row.value) || 0;
            oppsByOwner.set(row.owner, current);
        }
    }
            ...
            <div className="flex flex-wrap items-end justify-between gap-3">
                <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                        Kelola sales rep, distribusi lead, dan pipeline aktif per
                        orang.
                    </p>
                </div>
                <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
                    {allReps.filter((rep) => rep.is_active).length} aktif -{" "}
                    {allReps.length} total
                </span>
            </div>
                ...
                <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-400">
                        <tr>
                            ...
                            <th className="px-5 py-3 font-semibold">Lead Ditangani</th>
                    ...
                    <tbody className="divide-y divide-slate-100">
                                ...
                                <td colSpan={6} className="px-5 py-12 text-center">
                                    <div className="mx-auto max-w-xs">
                                        ...
                                        <p className="mt-1 text-xs text-slate-300">
                                            Tambahkan anggota tim pertama dari form di atas.
                                        </p>
                                    </div>
                                </td>
                            </tr>
                        ...
                        {allReps.map((rep) => {
                            ...
                            const opp = oppsByOwner.get(rep.name) ?? {
                                count: 0,
                                value: 0,
                            };
                            return (
                                <tr
                                    key={rep.id}
                                    className={`transition-colors hover:bg-slate-50/80 ${!rep.is_active ? "opacity-50" : ""}`}
                                >
                                    ...
                                    <td className="px-5 py-3">
                                        <span
                                            className={`rounded-full px-2.5 py-0.5 text-xs font-semibold capitalize ${ROLE_BADGE[rep.role] ?? ROLE_BADGE.sales}`}
                                        >
                                            {rep.role}
                                        </span>
                                    </td>
