...
export default async function OpportunitiesPage({
  ...
  const followUpGaps = open.filter(
    (opp) => !opp.next_action?.trim() || !opp.next_action_at,
  ).length;
  const ownerlessCount = all.filter((opp) => !opp.owner?.trim()).length;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-4 text-3xl font-black tracking-tight text-slate-950">
              Opportunities yang siap didorong ke proposal, negosiasi, lalu
              closed-won.
            </h1>
            <p className="mt-3 max-w-xl text-sm leading-6 text-slate-600">
              Pipeline penjualan per layanan, lengkap dengan forecast, target
              close, dan backlog owner yang masih perlu dibersihkan.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white/80 px-4 py-2.5 text-sm font-semibold text-slate-600 shadow-sm transition-colors hover:bg-slate-50"
            >
              <svg
                className="h-4 w-4"
                fill="none"
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-2xl border border-amber-200/80 bg-amber-50/90 p-5">
            ...
            <p className="mt-1 text-xs text-amber-800/70">
              {ownerlessCount} belum punya owner / {lostCount} lost
            </p>
          </div>
        </div>
      </section>

      <div className="rounded-[1.75rem] border border-slate-200/80 bg-white/95 p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-2">
          <Link
      ...
      <form
        ...
        className="flex flex-wrap items-center gap-2 rounded-[1.5rem] border border-slate-200/80 bg-slate-50/90 p-4"
      >
                  ...
                  <td colSpan={10} className="px-4 py-12 text-center">
                    <div className="mx-auto max-w-xs">
                      ...
                      <p className="mt-1 text-xs text-slate-300">
                        Konversi lead atau buat deal baru untuk membangun
                        pipeline.
                      </p>
                    </div>
                  </td>
