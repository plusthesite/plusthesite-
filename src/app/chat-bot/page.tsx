...
const testimonials = [
    { quote: "Collaborating with plus. was an exceptional journey. They truly grasped our goals and produced a solution that aligned perfectly with our vision.", name: "Mary Jones", role: "CEO, Laketown OH", avatar: "MJ" },
    { quote: "Working with plus. was a game changer. Their team crafted a platform that's both beautiful and intuitive, elevating our user engagement.", name: "Andrew Walker", role: "CTO, ITactics CA", avatar: "AW" },
    { quote: "Their expertise in blending technology with design truly impressed me. The platform transformed our digital engagement completely.", name: "Kate Doe", role: "CMO, Los Angeles CA", avatar: "KD" },
];
...
function ChatDemo({ p }: { p: any }) {
    ...
    useEffect(() => {
        ...
        chatMessages.forEach((msg, i) => {
            timers.push(setTimeout(() => setVisibleMessages((prev) => Math.max(prev, i + 1)), msg.delay));
        });
    ...
    return (
        <div className="mx-auto max-w-md rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xl overflow-hidden">
            <div className="flex items-center gap-3 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 px-5 py-4">
                ...
                <div>
                    <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{p.chatTitle}</p>
            ...
            <div className="h-80 overflow-y-auto p-4 space-y-3 bg-slate-50 dark:bg-slate-950/50">
                {chatMessages.slice(0, visibleMessages).map((msg, i) => (
                    <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`} style={{ animation: "fadeSlideUp 0.3s ease-out forwards" }}>
                        <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed whitespace-pre-line ${msg.role === "user" ? "bg-blue-600 text-white rounded-br-md" : "bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 rounded-bl-md"}`}>{msg.text}</div>
                    </div>
                ))}
            ...
            <div className="border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-3">
                <div className="flex items-center gap-2 rounded-full border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 px-4 py-2">
                    <input type="text" placeholder={p.chatPlaceholder} className="flex-1 bg-transparent text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 outline-none" readOnly />
